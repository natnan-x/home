<template>
  <div
    class="min-h-[var(--vh)] lg:px-25 max-w-6xl mx-auto flex flex-col justify-between md:justify-center"
  >
    <div
      class="flex-1 py-12 px-8 text-lg leading-relaxed text-left md:flex-none"
    >
...
<script setup>
  ...
  const startTypewriter = () => {
    ...
    const writer = new Typewriter(typeTarget.value, {
      loop: false,
      typeSpeed: 40,
      typeColor: localStorage.getItem("theme") === "dark" ? "white" : "black",
      cursorColor: localStorage.getItem("theme") === "dark" ? "white" : "black",
      deleteSpeed: 0,
    });
